...
| B-07 | RETOCAR | P1 | Backend | `indicadoresNegocio` tiene 4 campos hardcodeados en 0 (`ftePotencial`, `fcstRevPot`, `forecastCostPot`, `forecastCostos`, `difEstimacionCD`) | Datos incorrectos en la vista | `modules/pnl/pnl.service.ts:512` | Implementar lógica real: permitir cargar potencial por perfil genérico, FTEs, precio y costo editables por mes; el potencial afecta el P&L del proyecto: si no hay real, se suma al total; si hay real, el real manda; visualización/exportación igual que proyectos reales | Medio |
...
| B-29 | ~~RETOCAR~~ | P1 | UX / Épica Potencial | ✅ HECHO (2026-02-26) — Semántica merge-when-no-real: potencial suma al efectivo cuando no hay real | La sección amber separada (no-mezcla) era confusa: el usuario esperaba ver el potencial integrado en el total cuando no hay real | `pnl.service.ts`, `pnl.types.ts`, `rolling.types.ts`, `useRollingData.ts`, `useRollingAggregates.ts`, `RfActualsTable.tsx`, `RevenueTable.tsx`, `ProyectoPnlGrid.tsx` | Mes sin real + potencial activo → efectivo = asignado + potencial, badge "Pot.*"; mes con real → efectivo = real, badge "Real"; toggle controla inclusión; subfilas "Confirmado" + "Potencial*" en Rolling | Medio |
